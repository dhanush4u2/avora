            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4">
                  ...
                  <button onClick={onClose} className="font-body text-sm text-cream/60 underline underline-offset-4 hover:text-cream transition-colors">
                    Continue shopping
                  </button>
                </div>
              ) : (
                      ...
                      <li key={item.variantId} className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <button onClick={() => removeItem(item.variantId)} className="text-cream/30 hover:text-cream/70 transition-colors">
                              <X size={14} />
                            </button>
                          </div>
                          <div className="flex items-center justify-between mt-3">
                            <div className="flex items-center border border-cream/20">
                              <button onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center text-cream/60 hover:text-cream transition-colors">
                                <Minus size={12} />
                              </button>
            ...
            {items.length > 0 && (
              <div className="border-t border-cream/10 px-6 py-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-display text-lg text-cream">₹ {totalPrice.toLocaleString('en-IN')}</span>
                </div>
                <p className="font-body text-xs text-cream/40">Shipping & taxes calculated at checkout</p>
                <motion.button
